...
function generateSequence(gridSize, n, totalRounds) {
  const totalCells = gridSize * gridSize;
  const seq = [];
  // Ensure ~35% of rounds (after the first N) are matches
...
export default function NBackChallenge() {
  ...
  const [feedback, setFeedback] = useState(null); // "hit"|"miss"|"false"|"reject"
  const [resultData, setResultData] = useState(null);

  const seqRef = useRef({ sequence: [], matchSet: new Set() });
  ...
  const evaluateRound = useCallback(() => {
    ...
    // Short delay to show feedback, then advance
    setTimeout(() => {
      setFeedback(null);
      setRound((r) => r + 1);
    }, 350);
  ...
  /* ── Ghost positions (last N cells) for hints ── */
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 gap-6">
            ...
            return (
              <motion.div
                ...
                className={`aspect-square rounded-xl relative ${
                  isGhost && !isActive ? "ring-1 ring-cyan-500/20" : ""
                }`}
              >
                {isGhost && !isActive && (
        ...
        <div className="flex flex-col items-center gap-3">
          <motion.button
            ...
            className={`px-12 py-5 rounded-2xl text-xl font-black transition-all duration-200 ${
              responded
                ? feedback === "hit"
