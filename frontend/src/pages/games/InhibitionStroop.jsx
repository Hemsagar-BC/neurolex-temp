...
export default function InhibitionStroop() {
  ...
  const [feedback, setFeedback] = useState(null); // { type: "correct"|"wrong", color }
  ...
  return (
    <GameWrapper title="Inhibition Stroop" difficulty={difficulty} timeLimit={timeLimit} onTimeUp={handleTimeUp} onExit={handleGoHome} score={score}>
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 pt-3">
        ...
        <span className={`ml-4 text-sm font-bold px-2.5 py-0.5 rounded-full ${
          runningAcc >= 80 ? "bg-green-500/20 text-green-400" : runningAcc >= 60 ? "bg-yellow-500/20 text-yellow-400" : "bg-red-500/20 text-red-400"
        }`}>
          {runningAcc}%
        </span>
      </div>
