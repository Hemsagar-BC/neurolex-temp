...
function solvePuzzle(gridSize, dots) {
  ...
  function isEndpoint(ck, r, c) {
    const [r2, c2] = dots[ck][1];
    return r === r2 && c === c2;
  }

  function solve(colorIdx) {
...
function buildGrid(gridSize, dots) {
  const grid = [];
  for (let r = 0; r < gridSize; r++) {
    const row = [];
...
export default function DotConnector() {
  ...
  const [flows, setFlows] = useState({}); // colorKey -> [{row,col}]
  const [activeFlow, setActiveFlow] = useState(null); // { colorKey, path: [{row,col}] }
  const [currentPuzzle, setCurrentPuzzle] = useState(null);
  const [connectedPairs, setConnectedPairs] = useState(new Set());
  ...
  /* ─── pick puzzle ─── */
  const getPuzzle = useCallback((level, index) => {
  ...
  /* ─── init puzzle ─── */
  const initPuzzle = useCallback((level, pIndex) => {
  ...
  /* ─── check win ─── */
  function checkWin(currentFlows, puzzle) {
  ...
  const handlePointerMove = useCallback((e) => {
    ...
    if (cellData.isDot && cellData.colorKey === flow.colorKey) {
      ...
      if (!isStart) {
        ...
        const completedFlows = { ...flowsRef.current, [flow.colorKey]: [...flow.path] };
  ...
  const handlePointerUp = useCallback(() => {
    ...
    if (flow) {
      ...
      const currentFlows = { ...flowsRef.current, [flow.colorKey]: [...flow.path] };
      flowsRef.current = currentFlows;
      setFlows(currentFlows);
  ...
  /* ─── handle puzzle win ─── */
  const handlePuzzleWin = useCallback((completedFlows) => {
  ...
  const resetPuzzle = useCallback(() => {
    ...
    setRedraws((r) => r + 1);
    session.recordAttempt(false, Date.now() - puzzleStartTime);
