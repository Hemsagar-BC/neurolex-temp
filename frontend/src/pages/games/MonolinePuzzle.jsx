...
function getUntraversedNeighbors(nodeId, edges, traversedSet) {
  const out = [];
  edges.forEach((e) => {
...
export default function MonolinePuzzle() {
  ...
  const storedDiff = (() => {
    try {
      return parseInt(localStorage.getItem("monoline_difficulty"), 10) || 1;
    } catch {
      return 1;
    }
  })();
  ...
  const puzzleStartRef = useRef(null);

  /* current puzzle data (mutable working copy) */
  const [currentPuzzle, setCurrentPuzzle] = useState(null);
  const [traversed, setTraversed] = useState(new Set());
  const [path, setPath] = useState([]); // traversed node IDs in order
  ...
  useEffect(() => {
    ...
    const toXY = (pct) => (pct / 100) * canvasSize;
    ...
    const draw = () => {
      ...
      if (showHint && currentPuzzle.solution) {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        currentPuzzle.solution.forEach((nid, i) => {
      ...
      // draw edges
      currentPuzzle.edges.forEach((e) => {
        const key = edgeKey(e.from, e.to);
        const isTraversed = traversed.has(key);
        const nA = currentPuzzle.nodes.find((n) => n.id === e.from);
        const nB = currentPuzzle.nodes.find((n) => n.id === e.to);
        if (!nA || !nB) return;
      ...
      if (dragging.current && currentNode !== null && cursorPos) {
        const cNode = currentPuzzle.nodes.find((n) => n.id === currentNode);
        if (cNode) {
          ctx.beginPath();
              ...
              (_, i) => (
                <span
                  ...
                  className={`w-2 h-2 rounded-full ${
                    i < attemptCount ? "bg-orange-400" : "bg-gray-600"
                  }`}
                />
              )
            )}
